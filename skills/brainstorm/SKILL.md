---
name: brainstorm
description: "Use this skill for a live brainstorming meeting: take structured notes while the user thinks out loud, extract claims to fact-check, and produce a final report only when the user ends the session with `/brainstorm done` or the exact phrase `brainstorming done` (case-insensitive, normalized)."
---
...
- Finish + produce report (recommended): `/brainstorm done`
...
3) For each candidate line:
   ...
   - trim again
4) Match (case-insensitive) either:
   - `^/brainstorm\s+done\s*$`
...
- Condition: candidate contains `brainstorm` and `done` (any case) but fails both end-trigger regexes.
- Append one line to: `tmp/brainstorm-rejects.log`
  - timestamp
  - raw input
  - normalized candidate(s)
  - reject reason (e.g., “missing leading slash”, “extra text after done”)
- Reply with the exact allowed triggers:
  - `/brainstorm done`
  - `brainstorming done`
...
6. Confirm in chat where notes are saved + reminder to use `/brainstorm done`.
